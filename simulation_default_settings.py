...
RP0 = 0
# Reservoir population (1 in 10000 has the infection)
W0 = 1
# Susceptible population
SP0 = NP - (IP0 + AP0 + RP0)
# Rates
Beta = 0.5; Gamma = 0.1
# Mean incubation period was assumed to be 5.2 days
## both omegap and omegadashp considered to have the same period
omegap = 0.1923
omegadashp = 0.1923
# Mean infectious period was set to be 5.8 days
gammap = 0.1724
gammadashp = 0.1724
...
betap = 0.001
# Transmission rate from W to Sp
betaw = 0.001
...
def f(t, y):
    ...
    dSp = np - (mp * sp) - (bp * sp * (ip + (k * ap))) - (bw * sp * w)
    dEp = (bp * sp * (ip + (k * ap))) + (bw * sp * w) - ((1 - deltap) * omegap * ep) - (deltap * omegadashp * ep) - (mp * ep)
    dIp = ((1 - deltap) * omegap * ep) - ((gammap + mp) * ip)
    dAp = (deltap * omegadashp * ep) - ((gammadashp + mp) * ap)
    dRp = (gammap * ip) + (gammadashp * ap) - (mp * rp)
    dW = epsilon * (ip + (c * ap) - w)
    
    return dSp, dEp, dIp, dAp, dRp, dW
...
plt.figure()
plt.plot(sol.t, S, label = "Susceptibles")
...
plt.title("SIS model (Standard incidence).")
